...
async function checkMontyMode() {
  try {
    ...
    const data = await res.json();
    const modeEl = document.getElementById('montyMode');
    if (modeEl) modeEl.textContent = data.mode === 'ai' ? '🧠 AI Mode' : '⚡ Pattern Mode';
  } catch(e) {
    const modeEl = document.getElementById('montyMode');
    if (modeEl) modeEl.textContent = '⚠️ Offline';
  }
}

function montyQuick(text) {
  const input = document.getElementById('montyInput');
  if (input) { input.value = text; montySend(); }
}

function montyClear() {
  montyHistory = [];
  const msgs = document.getElementById('montyChatMessages');
  if (msgs) msgs.innerHTML = `
    <div class="monty-msg monty-msg-bot">
      <div class="monty-avatar">🤖</div>
      <div class="monty-bubble">Chat cleared. What do you need?</div>
    </div>`;
}
...
async function montySend() {
  ...
  try {
    const res = await fetch('/.netlify/functions/monty-admin', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: msg, context: montyHistory.slice(-10) })
    });
    const data = await res.json();
    ...
    if (data.results?.length) {
      ...
      for (const r of data.results) {
        ...
        const statusClass = r.success ? 'act-success' : 'act-fail';
        const label = r.action.replace(/_/g, ' ').toUpperCase();

        html += `<div class="monty-action-card">
          <div class="act-label">${label}</div>
          <span class="act-status ${statusClass}">${icon} ${r.success ? 'Done' : 'Failed'}</span>`;

        // Show details
        if (r.name) html += ` <span style="color:rgba(255,255,255,0.6);font-size:12px;">— ${r.name}</span>`;
        if (r.id) html += ` <span style="color:rgba(255,255,255,0.3);font-size:11px;">(${r.id})</span>`;
        if (r.title) html += ` <span style="color:rgba(255,255,255,0.6);font-size:12px;">— ${r.title}</span>`;
        if (r.contacts) {
          html += `<div style="margin-top:8px;">`;
          if (r.contacts.length === 0) {
            html += '<div style="color:rgba(255,255,255,0.4);font-size:12px;">No contacts found.</div>';
          } else {
            r.contacts.forEach(c => {
              html += `<div style="padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:13px;">
                <strong>${c.name || 'Unknown'}</strong> ${c.email ? '· ' + c.email : ''} ${c.phone ? '· ' + c.phone : ''}
                ${c.company ? '<br><span style="color:rgba(255,255,255,0.4);font-size:11px;">' + c.company + '</span>' : ''}
              </div>`;
            });
          }
          html += '</div>';
        ...
        if (r.jobs) {
          html += `<div style="margin-top:8px;">`;
          if (r.jobs.length === 0) {
            html += '<div style="color:rgba(255,255,255,0.4);font-size:12px;">No jobs found.</div>';
          } else {
            r.jobs.forEach(j => {
              const statusColors = { new: '#3b82f6', inprogress: '#f59e0b', review: '#a855f7', done: '#22c55e' };
              html += `<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:13px;display:flex;justify-content:space-between;align-items:center;">
                <div><strong>${j.title || j.client_name || 'Untitled'}</strong>
                  <span style="color:rgba(255,255,255,0.4);font-size:11px;"> · ${j.type || 'general'}</span>
                </div>
                <span style="padding:2px 10px;border-radius:10px;font-size:11px;font-weight:600;background:${statusColors[j.status] || '#555'}20;color:${statusColors[j.status] || '#888'};">${j.status}</span>
              </div>`;
            });
          }
          html += '</div>';
        }
        if (r.error) html += ` <div style="color:#ef4444;font-size:12px;margin-top:4px;">${r.error}</div>`;
    ...
    addMontyMessage(html, 'bot', true);
    montyHistory.push({ role: 'assistant', content: data.response });

  } catch(err) {
    removeMontyTyping(typingId);
    addMontyMessage(`⚠️ Error: ${err.message}. Make sure the Netlify function is deployed.`, 'bot', true);
  }
}
...
function addMontyMessage(content, type, isHtml) {
  const msgs = document.getElementById('montyChatMessages');
  if (!msgs) return;
  const div = document.createElement('div');
  div.className = `monty-msg monty-msg-${type}`;
  const avatar = type === 'bot' ? '🤖' : '👤';
  div.innerHTML = `<div class="monty-avatar">${avatar}</div><div class="monty-bubble">${isHtml ? content : escapeHtml(content)}</div>`;
