...
// GET /?cid=UUID&token=HASH — shows confirmation page
// POST /?cid=UUID&token=HASH — marks contact unsubscribed in Supabase
  ...
  if (event.httpMethod === 'POST') {
    ...
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/crm_contacts?id=eq.${cid}`, {
        ...
        body: JSON.stringify({ email_unsubscribed: true, email_unsubscribed_at: new Date().toISOString(), status: 'unsubscribed' })
      });
      // Also add to suppression list for SMS
