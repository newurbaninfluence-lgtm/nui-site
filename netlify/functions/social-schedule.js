// social-schedule.js — Schedule social posts via Monty
// Saves posts to Supabase, cron job on Mac Mini fires them at the right time
// Endpoints: POST /schedule (save), GET /schedule (list), POST /schedule/fire (fire due posts)
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // ── FIRE DUE POSTS (called by Mac Mini cron) ──
      ...
      for (const post of (due || [])) {
        try {
          ...
          await sbFetch(`scheduled_posts?id=eq.${post.id}`, {
            method: 'PATCH',
            body: JSON.stringify({ status: 'posted', posted_at: new Date().toISOString(), results })
          });
    ...
    if (event.httpMethod === 'POST') {
      ...
      const post = await sbFetch('scheduled_posts', {
        method: 'POST',
        body: JSON.stringify({
          caption, platform: platform || 'facebook',
          image_url: image_url || null,
          scheduled_for: new Date(scheduled_for).toISOString(),
          status: 'scheduled', source: source || 'monty',
