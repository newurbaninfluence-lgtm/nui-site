// ========================================================================
// push-reinvite.js  -  Admin-triggered SMS that links recipients back to
//                      the site with ?push=1 so the opt-in banner fires.
// ========================================================================
// Request:
//   POST /.netlify/functions/push-reinvite
//   Headers: X-Admin-Token
//   Body: { recipients: [{name, phone}], message_override?: string }
// Response:
//   { success: true, sent: N, failed: N, errors: [...] }
// ========================================================================
...
// Default re-invite copy. {name} is substituted per recipient.
const DEFAULT_MSG =
...
exports.handler = async (event) => {
  ...
  const template = (body.message_override && body.message_override.length > 20)
  ...
  for (const r of recipients) {
    ...
    const name = (r.name || 'there').split(/\s+/)[0]; // first name only
