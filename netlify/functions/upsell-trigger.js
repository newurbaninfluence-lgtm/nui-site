// upsell-trigger.js — Netlify Scheduled Function
// Runs daily at 10am ET — checks for clients at 90-day mark
// Sends Faren an upsell prompt email + sends client a value-check email
...
// Upsell paths by current service
const UPSELL_PATHS = {
...
exports.handler = async () => {
  ...
  try {
    // Find clients whose upsell_trigger_date is today (within 24hr window)
    ...
    for (const client of clients) {
      ...
      // 1. Email Faren — prompt to reach out
      await sendMail(
        ADMIN_EMAIL,
        `💡 90-Day Upsell: ${client.name || client.business_name} — Offer ${upsell.next}`,
...
<h2 style="margin:0;color:#000;font-size:18px;font-weight:800;">💡 90-Day Upsell Trigger</h2>
...
<p style="color:#555;font-size:12px;text-align:center;margin:0;">A client email has also been sent to ${client.email} checking in on their results.</p>
      ...
      // 2. Email client — value check-in + soft introduction to next step
      if (client.email) {
        await sendMail(
          client.email,
...
<p style="color:#555;font-size:12px;text-align:center;">New Urban Influence · Detroit, MI · (248) 487-8747</p>
</div>`
        );
      }

      // 3. Log upsell attempt in Supabase
      await fetch(`${SUPABASE_URL}/rest/v1/activity_log`, {
        method: 'POST',
        headers: { ...sbHeaders, 'Prefer': 'return=minimal' },
        body: JSON.stringify({
          event_type: 'upsell_triggered',
          metadata: { client_id: client.id, current_service: client.current_service, upsell_offered: upsell.next, triggered_at: new Date().toISOString() }
        })
      }).catch(e => console.warn('Activity log failed:', e.message));

      // 4. Clear trigger date so it doesn't re-fire
