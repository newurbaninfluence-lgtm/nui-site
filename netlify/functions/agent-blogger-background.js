// agent-blogger-background.js — Background version for manual admin triggers
// Netlify background functions: 15-min timeout, returns 202 immediately
// Results written to agent_logs + blog_posts in Supabase
// Poll agent_logs?agent_id=eq.blogger to see when done
// Same logic as agent-blogger.js (scheduled), just wrapped for background execution
...
exports.handler = async (event) => {
  ...
  // Return 202 immediately — Netlify background functions process async
  // Client polls agent_logs?agent_id=eq.blogger to detect completion
  ...
  try {
    ...
    if (mode === 'batch') {
      ...
      let topicObj;
      if (body.topic) {
        topicObj = { topic: body.topic, category: body.category || 'Branding', keywords: body.keywords || ['Detroit branding', 'NUI agency'] };
      } else {
    ...
    await logRun({ success: false, error: err.message }).catch(() => {});
  }

  // Background functions always return 202
  return { statusCode: 202, headers: CORS, body: JSON.stringify({ queued: true, message: 'Blog generation started. Check Agent Logs for results.' }) };
};
