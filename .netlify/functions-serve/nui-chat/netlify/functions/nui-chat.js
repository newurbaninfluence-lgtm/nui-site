...
// netlify/functions/nui-chat.js
var fetch2 = require_lib2();
var { createClient } = require_dist4();
var SYSTEM_PROMPT = `You are Monty, the AI assistant for New Urban Influence (NUI), a boutique branding and design agency based in Detroit, Michigan. You help visitors understand NUI's services, recommend the right solutions, and guide them toward getting started.
...
BUNDLE PACKAGES (15% savings vs \xE0 la carte):
...
var EXTRACT_PROMPT = `Analyze this conversation between a website visitor and Monty (NUI's AI assistant). Extract any contact information the visitor shared and assess their lead quality.
...
async function extractAndSaveLead(apiKey, messages, sessionId) {
  ...
  if (!url || !key) return;
  const convoText = messages.filter((m) => m.role === "user" || m.role === "assistant").map((m) => `${m.role === "user" ? "Visitor" : "Monty"}: ${m.content}`).join("\n");
  const extractRes = await fetch2("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-api-key": apiKey,
  ...
  if (existing) {
    ...
    const { data: created, error } = await supabase.from("crm_contacts").insert(newContact).select().single();
    if (created) {
...
//# sourceMappingURL=nui-chat.js.map
