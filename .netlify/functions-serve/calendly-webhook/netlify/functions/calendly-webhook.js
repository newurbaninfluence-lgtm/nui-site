...
async function triggerBookingConfirmation(clientName, clientEmail, clientPhone, serviceInterest, meetingDate, meetingTime, zoomLink) {
  ...
  if (clientEmail && SMTP_USER) {
    try {
      ...
      console.warn("[Calendly] Confirm email failed:", e.message);
    }
  }
  if (clientPhone && OP_KEY && OP_FROM) {
    try {
      ...
      console.warn("[Calendly] Confirm SMS failed:", e.message);
    }
  }
}
async function triggerCancellationNotice(clientName, clientEmail, clientPhone, meetingDate, meetingTime) {
  ...
  if (clientEmail && SMTP_USER) {
    try {
      ...
      console.warn("[Calendly] Cancel email failed:", e.message);
    }
  }
  if (clientPhone && OP_KEY && OP_FROM) {
    try {
      ...
      console.warn("[Calendly] Cancel SMS failed:", e.message);
    }
  }
}
    ...
    if (eventType === "invitee.canceled") {
      ...
      if (matches?.length > 0) {
        await fetch(`${SUPABASE_URL}/rest/v1/meetings?id=eq.${matches[0].id}`, {
          method: "PATCH",
          headers: supaHeaders,
          body: JSON.stringify({ status: "canceled", outcome: "canceled", updated_at: (/* @__PURE__ */ new Date()).toISOString() })
        });
...
function extractPhone(qAndA) {
  ...
  const phoneQ = qAndA.find(
    (q) => q.question?.toLowerCase().includes("phone") || q.question?.toLowerCase().includes("number") || q.question?.toLowerCase().includes("cell")
  );
  return phoneQ?.answer || "";
}
function extractService(qAndA) {
  if (!Array.isArray(qAndA)) return "";
  const serviceQ = qAndA.find(
    (q) => q.question?.toLowerCase().includes("service") || q.question?.toLowerCase().includes("interest") || q.question?.toLowerCase().includes("looking for") || q.question?.toLowerCase().includes("project") || q.question?.toLowerCase().includes("help with")
  );
...
//# sourceMappingURL=calendly-webhook.js.map
