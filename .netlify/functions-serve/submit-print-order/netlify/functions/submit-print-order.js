...
${brand.company_website ? `Web: ${brand.company_website}` : ""}`;
    ...
    module2.exports = { getBrand: getBrand2, getTransporter, getFromAddress: getFromAddress2, hasSMTP, hasOpenPhone, buildEmailFooter, buildEmailSignature, buildSmsSystemPrompt, NUI_DEFAULTS };
  }
});
...
// netlify/functions/submit-print-order.js
var nodemailer = require("nodemailer");
var { getBrand, getFromAddress } = require_agency_brand();
exports.handler = async (event) => {
  ...
  try {
    ...
    if (SB_URL && SB_KEY) {
      const fetch2 = (...args) => Promise.resolve().then(() => __toESM(require_lib2())).then(({ default: f }) => f(...args));
      await (await fetch2)(`${SB_URL}/rest/v1/print_requests`, {
        method: "POST",
        headers: {
          "apikey": SB_KEY,
      ...
      await (await fetch2)(`${SB_URL}/rest/v1/activity_log`, {
        method: "POST",
        headers: {
          "apikey": SB_KEY,
          "Authorization": `Bearer ${SB_KEY}`,
    ...
    const itemsHtml = items.map((item) => `
      <tr>
        ...
        <td style="padding:10px;border-bottom:1px solid #222;color:#fff;text-align:center">
          ${item.needsDesign ? '<span style="color:#dc2626">\u{1F3A8} Design+Print</span>' : '<span style="color:#22c55e">\u2705 Print Only</span>'}
        </td>
    ...
    const vendorHtml = Object.entries(vendorSummary).map(
      ([v, prods]) => `<strong>${v === "signs365" ? "Signs365" : "Knello"}:</strong> ${prods.join(", ")}`
    ).join("<br>");
    const addressHtml = shipping_address ? `${shipping_address.address || ""}, ${shipping_address.city || ""}, ${shipping_address.state || "MI"} ${shipping_address.zip || ""}` : "Not provided";
    ...
    await transporter.sendMail({
      ...
      subject: `\u{1F5A8}\uFE0F New Print Order: $${total} \u2014 ${client_name}`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:650px;margin:0 auto;background:#0a0a0a;color:#fff;padding:32px;border-radius:12px;border:1px solid #222">
          ...
          <p style="color:#666;margin:0 0 24px">Submitted ${(/* @__PURE__ */ new Date()).toLocaleString("en-US", { timeZone: "America/Detroit" })}</p>
    ...
    const clientItemsHtml = items.map((item) => `
      <div style="display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
        <div>
          ...
          <div style="font-size:12px;margin-top:2px">${item.needsDesign ? "\u{1F3A8} Design + Print" : "\u2705 Print Only"}</div>
        </div>
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#111;color:#fff;padding:32px;border-radius:12px">
          ...
          <h2 style="text-align:center;margin-bottom:8px">Order Received! \u{1F389}</h2>
          ...
          <p style="color:#666;font-size:12px;text-align:center">${brand.agency_name}${brand.company_city ? " \xB7 " + brand.company_city : ""}</p>
        </div>
      `
    });
    return {
...
//# sourceMappingURL=submit-print-order.js.map
