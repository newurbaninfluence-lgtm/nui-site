...
var require_agency_brand = __commonJS({
  "netlify/functions/utils/agency-brand.js"(exports2, module2) {
    ...
    function buildEmailFooter(brand2) {
      return `
    ...
    function buildEmailSignature2(brand2) {
      return `
...
${brand2.company_website ? `Web: ${brand2.company_website}` : ""}`;
    ...
    module2.exports = { getBrand: getBrand2, getTransporter: getTransporter2, getFromAddress: getFromAddress2, hasSMTP, hasOpenPhone, buildEmailFooter, buildEmailSignature: buildEmailSignature2, buildSmsSystemPrompt, NUI_DEFAULTS };
  }
});

// netlify/functions/visitor-auto-email.js
var nodemailer = require("nodemailer");
var { createClient } = require_dist4();
var { getBrand, getFromAddress, getTransporter, buildEmailSignature } = require_agency_brand();
...
async function sendVisitorEmail(visitorId, currentUrl) {
  const { data: visitor, error: vErr } = await supabase.from("identified_visitors").select("*").eq("id", visitorId).single();
  if (vErr || !visitor) {
  ...
  const cooldownDate = new Date(Date.now() - 7 * 24 * 60 * 60 * 1e3).toISOString();
  const { data: recentEmails } = await supabase.from("visitor_auto_emails").select("id").eq("visitor_id", visitorId).gte("sent_at", cooldownDate).limit(1);
  if (recentEmails && recentEmails.length > 0) {
    console.log("Cooldown active for:", visitor.business_email);
    return { sent: false, reason: "cooldown_active" };
  }
  const { data: pageViews } = await supabase.from("visitor_page_views").select("captured_url, seen_at").eq("visitor_id", visitorId).order("seen_at", { ascending: true });
  ...
  if (!SMTP_USER && !brand2.smtp_user) {
    console.error("SMTP not configured");
    return { sent: false, reason: "smtp_not_configured" };
  }
  const transporter = getTransporter(brand2);
  ...
  if (visitor.status === "new") {
    await supabase.from("identified_visitors").update({ status: "contacted", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", visitorId);
  }
...
//# sourceMappingURL=visitor-auto-email.js.map
