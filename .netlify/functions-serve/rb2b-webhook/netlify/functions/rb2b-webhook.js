...
function triggerAutoEmail(visitorId, capturedUrl) {
  ...
  fetch2(`${siteUrl}/.netlify/functions/visitor-auto-email`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ visitor_id: visitorId, captured_url: capturedUrl })
  }).then((r) => console.log("Auto-email trigger:", r.status)).catch((e) => console.warn("Auto-email trigger failed:", e.message));
}
exports.handler = async (event) => {
  ...
  try {
    ...
    const visitor = {
      ...
      seen_at: data["Seen At"] || (/* @__PURE__ */ new Date()).toISOString(),
    ...
    if (visitor.linkedin_url) {
      const { data: found } = await supabase.from("identified_visitors").select("id, visit_count").eq("linkedin_url", visitor.linkedin_url).single();
      existing = found;
    }
    if (!existing && visitor.business_email) {
      const { data: found } = await supabase.from("identified_visitors").select("id, visit_count").eq("business_email", visitor.business_email).single();
      existing = found;
    }
    if (existing) {
      const { error } = await supabase.from("identified_visitors").update({
      ...
      const { data: inserted, error } = await supabase.from("identified_visitors").insert({
        ...visitor,
        last_seen_at: visitor.seen_at,
        last_captured_url: visitor.captured_url,
        last_referrer: visitor.referrer,
...
//# sourceMappingURL=rb2b-webhook.js.map
