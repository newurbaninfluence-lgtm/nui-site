// netlify/functions/agent-blogger-background.js
var CORS = {
...
var SYNTH = process.env.SYNTHESYS_API_KEY;
var PEXELS = process.env.PEXELS_API_KEY;
...
- Opening hook that grabs attention immediately (no "In today's world" clich\xE9s)
...
async function logRun(data) {
  try {
    await sbFetch("agent_logs", {
      method: "POST",
      body: JSON.stringify({ agent_id: "blogger", status: data.success ? "success" : "error", metadata: data, created_at: (/* @__PURE__ */ new Date()).toISOString() })
    });
  } catch {
  }
}
...
<div class="blog-voice-overview" style="background:#1a1a1a;border:1px solid #333;border-radius:12px;padding:20px 24px;margin:24px 0;">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
    ...
    <strong style="color:#D4A843;font-size:14px;">\u{1F399} Listen to the Overview</strong>
  </div>
...
//# sourceMappingURL=agent-blogger-background.js.map
