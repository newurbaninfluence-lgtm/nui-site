  ...
  try {
      ...
      if (dbRecord.source === "booking_page" && dbRecord.client_phone) {
        ...
        if (!contacts?.length) {
          ...
          await fetch(`${SUPABASE_URL}/rest/v1/crm_contacts?id=eq.${contacts[0].id}`, {
            method: "PATCH",
            headers: { ...headers, "Prefer": "return=minimal" },
            body: JSON.stringify({ status: "qualified", lead_score: 8, last_activity_at: (/* @__PURE__ */ new Date()).toISOString() })
          }).catch(() => {
          });
        }
      }
    ...
    if (event.httpMethod === "PATCH") {
      ...
      for (const key of allowedFields) {
        if (updateData[key] !== void 0) patch[key] = updateData[key];
      }
...
//# sourceMappingURL=save-booking.js.map
