...
// netlify/functions/classify-contacts.js
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { data: contacts, error: fetchErr } = await supabase.from("crm_contacts").select("id, company, notes, industry").is("business_type", null).not("company", "is", null).neq("company", "").order("created_at", { ascending: true }).limit(batchSize);
    if (fetchErr) throw fetchErr;
    const { count: remaining } = await supabase.from("crm_contacts").select("id", { count: "exact", head: true }).is("business_type", null).not("company", "is", null).neq("company", "");
    ...
    for (const c of classifications) {
      if (!c.id || !validTypes.includes(c.business_type)) {
        failed++;
        continue;
      }
      const category = (c.business_category || "other").toLowerCase().replace(/[^a-z_]/g, "_");
      if (!dryRun) {
        const { error: updErr } = await supabase.from("crm_contacts").update({
...
//# sourceMappingURL=classify-contacts.js.map
