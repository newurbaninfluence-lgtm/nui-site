// netlify/functions/agent-creator.js
...
var SYNTH_KEY = process.env.SYNTHESYS_API_KEY;
...
async function saveDraft(data) {
  return await sbFetch("content_drafts", { method: "POST", body: JSON.stringify({ ...data, status: "pending_approval", created_at: (/* @__PURE__ */ new Date()).toISOString() }) });
}
...
${draft.hashtags}`, image_url: draft.image_url, audio_url: draft.audio_url, platform: platform || "facebook", status: "scheduled", scheduled_for: scheduled_for || new Date(Date.now() + 36e5).toISOString(), created_by: "agent_creator" }) });
      await sbFetch(`content_drafts?id=eq.${draft_id}`, { method: "PATCH", body: JSON.stringify({ status: "approved", approved_at: (/* @__PURE__ */ new Date()).toISOString() }) });
    ...
    for (const item of topics) {
      ...
      if (content.error) {
        batchResults.push({ error: content.error, topic: item.topic });
        continue;
      }
