...
var require_agency_brand = __commonJS({
  "netlify/functions/utils/agency-brand.js"(exports2, module2) {
    ...
    function buildSmsSystemPrompt2(brand) {
...
${brand.company_website ? `Web: ${brand.company_website}` : ""}`;
    ...
    module2.exports = { getBrand: getBrand2, getTransporter, getFromAddress: getFromAddress2, hasSMTP, hasOpenPhone, buildEmailFooter, buildEmailSignature, buildSmsSystemPrompt: buildSmsSystemPrompt2, NUI_DEFAULTS };
  }
});
...
var require_security = __commonJS({
  "netlify/functions/utils/security.js"(exports2, module2) {
    ...
    function getCorsHeaders2(event) {
    ...
    function handleOptions2(event) {
      if (event.httpMethod === "OPTIONS") {
        return { statusCode: 204, headers: getCorsHeaders2(event), body: "" };
      }
      return null;
    }
    module2.exports = {
      getCorsHeaders: getCorsHeaders2,
...
// netlify/functions/monty-admin.js
var { getBrand, getFromAddress, buildSmsSystemPrompt } = require_agency_brand();
var SYSTEM_PROMPT = `You are Monty, the AI assistant for New Urban Influence (NUI), a Detroit-based branding and print agency.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (ANTHROPIC_KEY) {
      try {
        const fetchMod2 = await Promise.resolve().then(() => __toESM(require_lib2()));
        const nfetch2 = fetchMod2.default;
        const aiRes = await nfetch2("https://api.anthropic.com/v1/messages", {
          method: "POST",
          headers: {
    ...
    const results = [];
    const fetchMod = await Promise.resolve().then(() => __toESM(require_lib2()));
        ...
        switch (action.action) {
          ...
          case "create_brand_guide": {
            ...
            const data = await sbFetch("brand_guides", {
              ...
              body: JSON.stringify({
                ...
                client_id: d.client_id || null,
                job_id: d.job_id || null,
                business_name: d.business_name || d.client_name,
                industry: d.industry || "",
                status: "draft",
                sections: {},
                created_at: (/* @__PURE__ */ new Date()).toISOString()
          ...
          case "add_print_order": {
            ...
            const data = await sbFetch("print_requests", {
              ...
              body: JSON.stringify({
                ...
                product_name: d.product || d.product_name,
                notes: d.details || d.quantity || "",
          ...
          case "update_job_status": {
            ...
            await sbFetch(`jobs?id=eq.${d.job_id}`, {
              method: "PATCH",
              body: JSON.stringify({ status: d.status, updated_at: (/* @__PURE__ */ new Date()).toISOString() })
            });
          ...
          case "send_sms": {
            ...
            await sbFetch("activity_log", {
              ...
              body: JSON.stringify({
                ...
                metadata: { from: fromNumber, to: d.to, source: "monty", sent_at: (/* @__PURE__ */ new Date()).toISOString() },
                created_at: (/* @__PURE__ */ new Date()).toISOString()
              })
            });
          ...
          case "write_blog_post": {
            ...
            if (!ANTHROPIC_KEY2) throw new Error("ANTHROPIC_API_KEY not set");
...
Content must be helpful, NUI-branded, and end with a soft CTA toward booking a strategy call.`;
            const blogAI = await nfetch("https://api.anthropic.com/v1/messages", {
              method: "POST",
              headers: { "Content-Type": "application/json", "x-api-key": ANTHROPIC_KEY2, "anthropic-version": "2023-06-01" },
              body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 2e3, messages: [{ role: "user", content: blogPrompt }] })
            });
            ...
            let blogPost;
            try {
              blogPost = JSON.parse(jsonMatch ? jsonMatch[1] : blogText);
            } catch (e) {
              throw new Error("Blog content generation failed: invalid JSON from AI");
            }
            const slug = (blogPost.title || d.title || d.topic).toLowerCase().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").slice(0, 80);
            const postId = "blog_" + Date.now();
            const row = {
              id: postId,
              slug,
              title: blogPost.title || d.title,
          ...
          case "generate_image": {
            ...
            const uploadRes = await nfetch(`${SB_URL2}/storage/v1/object/generated-images/${fileName}`, {
              method: "POST",
              headers: {
            ...
            const imageUrl = `${SB_URL2}/storage/v1/object/public/generated-images/${fileName}`;
            let posted = [];
...
var { getCorsHeaders, requireAdmin, handleOptions } = require_security();
var CORS_HEADERS = {
...
//# sourceMappingURL=monty-admin.js.map
