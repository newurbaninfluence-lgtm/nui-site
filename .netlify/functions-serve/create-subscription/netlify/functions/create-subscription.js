...
${brand.company_website ? `Web: ${brand.company_website}` : ""}`;
    ...
    module2.exports = { getBrand: getBrand2, getTransporter, getFromAddress, hasSMTP, hasOpenPhone, buildEmailFooter, buildEmailSignature, buildSmsSystemPrompt, NUI_DEFAULTS };
  }
});

// netlify/functions/create-subscription.js
var { requireAdmin } = require_security();
var { getBrand } = require_agency_brand();
var CORS_HEADERS = {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (billingType && billingType !== "one_time") {
      ...
      const sessionParams2 = new URLSearchParams();
      sessionParams2.append("mode", "subscription");
      sessionParams2.append("line_items[0][price]", price.id);
      sessionParams2.append("line_items[0][quantity]", "1");
      sessionParams2.append("success_url", `${origin}/app#portal?payment=success`);
      sessionParams2.append("cancel_url", `${origin}/app#portal?payment=cancel`);
      sessionParams2.append("metadata[clientId]", clientId || "");
      sessionParams2.append("metadata[invoiceId]", invoiceId || "");
      sessionParams2.append("metadata[billingType]", billingType);
      if (clientEmail) sessionParams2.append("customer_email", clientEmail);
      const sessionResp2 = await fetch("https://api.stripe.com/v1/checkout/sessions", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${stripeKey}`,
          "Content-Type": "application/x-www-form-urlencoded"
        },
        body: sessionParams2.toString()
      });
      const session2 = await sessionResp2.json();
      if (!sessionResp2.ok) throw new Error(session2.error?.message || "Session creation failed");
      return {
        statusCode: 200,
        headers: CORS_HEADERS,
        body: JSON.stringify({ url: session2.url, sessionId: session2.id, type: "subscription" })
      };
    }
