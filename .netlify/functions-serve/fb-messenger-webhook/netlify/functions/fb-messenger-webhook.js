// netlify/functions/fb-messenger-webhook.js
var CORS_HEADERS = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "Content-Type",
  "Access-Control-Allow-Methods": "GET, POST, OPTIONS"
};
var MONTY_SYSTEM_PROMPT = `You are Monty, the AI assistant for New Urban Influence (NUI), a branding and design agency in Detroit, Michigan. You're responding to Facebook or Instagram DMs.
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === "GET") {
    ...
    if (params["hub.mode"] === "subscribe" && params["hub.verify_token"] === process.env.FB_VERIFY_TOKEN) {
  ...
  try {
    ...
    for (const entry of body.entry || []) {
      for (const msg of entry.messaging || []) {
        ...
        console.log(`\u{1F4AC} FB DM from ${senderId}: "${text}"`);
      ...
      for (const change of entry.changes || []) {
        ...
        console.log(`\u{1F4AC} FB Comment from ${commentorId}: "${val.message}"`);
...
async function handleDM({ senderId, text, timestamp, PAGE_TOKEN, PAGE_ID, API_KEY, SUPA_URL, SUPA_KEY, channel }) {
  ...
  saveToSupabase({ senderId, incomingText: text, reply, channel, SUPA_URL, SUPA_KEY }).catch(
    (e) => console.warn("Supabase save failed:", e.message)
  );
}
async function replyToComment({ commentId, text, PAGE_TOKEN, API_KEY, SUPA_URL, SUPA_KEY }) {
  ...
  saveToSupabase({ senderId: null, incomingText: text, reply, channel: "facebook_comment", SUPA_URL, SUPA_KEY }).catch((e) => console.warn("Supabase comment save failed:", e.message));
}
async function getMontyReply(text, API_KEY, isComment = false) {
  ...
  const contextNote = isComment ? "This is a public Facebook comment \u2014 keep reply brief and invite them to DM for details." : "This is a private DM.";
  try {
...
//# sourceMappingURL=fb-messenger-webhook.js.map
