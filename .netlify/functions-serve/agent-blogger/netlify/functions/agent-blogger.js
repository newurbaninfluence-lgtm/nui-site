// netlify/functions/agent-blogger.js
...
Return ONLY valid JSON \u2014 no markdown, no fences, no explanation:
{
  ...
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 3e3, messages: [{ role: "user", content: prompt }] })
  });
  ...
  try {
    ...
    throw new Error("Blog JSON parse failed: " + raw.slice(0, 200));
  }
}
...
async function logRun(data) {
  try {
    await sbFetch("agent_logs", { method: "POST", body: JSON.stringify({ agent_id: "blogger", status: data.success ? "success" : "error", metadata: data, created_at: (/* @__PURE__ */ new Date()).toISOString() }) });
  } catch {
  }
}
async function generateAndSave(topicObj, autoPublish = false) {
  ...
  if (voiceover?.audio_url) {
    const audioBlock = `<div class="blog-voice-overview" style="background:#1a1a1a;border:1px solid #333;border-radius:12px;padding:20px 24px;margin:24px 0;"><div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;"><strong style="color:#C9A227;font-size:14px;">\u{1F399} Listen to the Overview</strong></div><audio controls style="width:100%;height:44px;" preload="none"><source src="${voiceover.audio_url}" type="audio/mpeg"></audio><p style="color:#888;font-size:12px;margin:8px 0 0;">60-second audio overview by Faren Young</p></div>`;
...
//# sourceMappingURL=agent-blogger.js.map
