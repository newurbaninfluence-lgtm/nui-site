...
// netlify/functions/smart-lists.js
...
async function fetchMatching(filters, limit = 1e3) {
  let q = supabase.from("crm_contacts").select("id, first_name, last_name, email, phone, company, business_type, business_category, status, tags").order("last_activity_at", { ascending: false, nullsFirst: false }).limit(Math.min(limit, 5e3));
  q = applyFilters(q, filters);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === "list") {
      const { data, error } = await supabase.from("smart_lists").select("*").order("created_at", { ascending: false });
      if (error) throw error;
    ...
    if (action === "create") {
      ...
      const { data, error } = await supabase.from("smart_lists").insert({
        name: body.name,
        description: body.description || null,
        filters,
        icon: body.icon || "\u{1F4CB}",
    ...
    if (action === "update") {
      ...
      if (body.filters !== void 0) {
        patch.filters = body.filters;
        patch.contact_count = await countMatching(body.filters);
        patch.last_refreshed_at = (/* @__PURE__ */ new Date()).toISOString();
      }
      const { data, error } = await supabase.from("smart_lists").update(patch).eq("id", body.id).select().single();
      if (error) throw error;
      return { statusCode: 200, headers, body: JSON.stringify({ list: data }) };
    }
    if (action === "delete") {
    ...
    if (action === "match") {
      ...
      if (body.id) {
        ...
        await supabase.from("smart_lists").update({ last_used_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", body.id);
      }
      const contacts = await fetchMatching(filters || {}, body.limit || 1e3);
    ...
    if (action === "refresh") {
      ...
      const { data, error } = await supabase.from("smart_lists").update({ contact_count: count, last_refreshed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", body.id).select().single();
      if (error) throw error;
      return { statusCode: 200, headers, body: JSON.stringify({ list: data }) };
    }
