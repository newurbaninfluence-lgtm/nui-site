...
// netlify/functions/verify-contact.js
var { createClient } = require_dist4();
var supabase = createClient(
  process.env.SUPABASE_URL,
  process.env.SUPABASE_SERVICE_KEY
);
var QEV_KEY = process.env.QUICKEMAILVERIFICATION_API_KEY;
var VERIPHONE_KEY = process.env.VERIPHONE_API_KEY;
async function verifyEmail(email) {
  ...
  try {
    ...
    return {
      ...
      reason: data.reason,
      // rejected_email, invalid_domain, etc.
      disposable: data.disposable === "true",
      role_based: data.role === "true",
      // info@, admin@, etc.
...
async function verifyPhone(phone) {
  ...
  try {
    ...
    return {
      ...
      sms_safe: smsable,
      // true only if mobile — don't SMS landlines/voip
...
async function saveResults(contact_id, emailResult, phoneResult) {
  ...
  const update = {
    ...
    email_risk: emailResult?.result ?? null,
    // valid/invalid/unknown
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (batch) {
      const { data: contacts } = await supabase.from("crm_contacts").select("id, email, phone, full_name").is("verified_at", null).limit(100);
      ...
      for (const contact of contacts) {
        ...
        const [emailResult2, phoneResult2] = await Promise.all([
          verifyEmail(contact.email),
          verifyPhone(contact.phone)
        ]);
        await saveResults(contact.id, emailResult2, phoneResult2);
        results.push({
          id: contact.id,
          name: contact.full_name,
          email: contact.email,
          phone: contact.phone,
          email_safe: emailResult2?.safe_to_send,
          phone_sms_safe: phoneResult2?.sms_safe,
          email_result: emailResult2?.result,
          phone_type: phoneResult2?.phone_type
        });
      }
...
//# sourceMappingURL=verify-contact.js.map
