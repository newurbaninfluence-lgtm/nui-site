...
var OP_KEY = process.env.OPENPHONE_API_KEY;
var OP_FROM = process.env.OPENPHONE_PHONE_NUMBER;
...
<h2 style="margin:0;font-size:22px;font-weight:800;letter-spacing:-0.5px;">\u{1F514} New Service Inquiry</h2>
<p style="margin:6px 0 0;opacity:0.85;font-size:14px;">${(/* @__PURE__ */ new Date()).toLocaleString("en-US", { timeZone: "America/Detroit" })}</p>
</div>
...
<div style="background:#111;border-radius:8px;padding:20px 24px;margin-bottom:20px;">
<h3 style="margin:0 0 12px;font-size:14px;text-transform:uppercase;letter-spacing:2px;color:#C9A227;">\u{1F511} Credentials Needed</h3>
<ul style="margin:0;padding:0;list-style:none;">${credsHTML}</ul>
</div>
<div style="background:#111;border-radius:8px;padding:20px 24px;margin-bottom:20px;">
<h3 style="margin:0 0 8px;font-size:14px;text-transform:uppercase;letter-spacing:2px;color:#C9A227;">\u{1F4CA} Day 30 Report to Send Client</h3>
...
<h3 style="margin:0 0 6px;font-size:13px;text-transform:uppercase;letter-spacing:2px;color:#C9A227;">\u{1F4A1} 90-Day Upsell Path</h3>
...
<a href="https://newurbaninfluence.com/app/#clients" style="color:#D90429;font-size:13px;font-weight:700;letter-spacing:1px;text-decoration:none;text-transform:uppercase;">Open Admin Portal \u2192</a>
  ...
  try {
    ...
    await mailer.sendMail({
      ...
      to: ADMIN_EMAIL,
      subject: `\u{1F514} New Inquiry: ${checklist.label} \u2014 ${submission.contact_name || "Unknown"} (${submission.business_name || ""})`,
      html
    });
...
async function markReplied(submissionId, results) {
  await fetch(`${SUPABASE_URL}/rest/v1/submissions?id=eq.${submissionId}`, {
    ...
    body: JSON.stringify({ auto_replied: true, auto_replied_at: (/* @__PURE__ */ new Date()).toISOString(), reply_results: results })
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (data.clientId) {
      const upsellDate = /* @__PURE__ */ new Date();
      ...
      fetch(`${SUPABASE_URL}/rest/v1/clients?id=eq.${data.clientId}`, {
        ...
        body: JSON.stringify({
          ...
          service_started_at: (/* @__PURE__ */ new Date()).toISOString()
        })
...
//# sourceMappingURL=save-submission.js.map
