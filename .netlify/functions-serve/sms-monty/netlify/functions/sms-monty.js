...
${brand.company_website ? `Web: ${brand.company_website}` : ""}`;
    ...
    module2.exports = { getBrand: getBrand2, getTransporter, getFromAddress, hasSMTP, hasOpenPhone, buildEmailFooter, buildEmailSignature, buildSmsSystemPrompt: buildSmsSystemPrompt2, NUI_DEFAULTS };
  }
});
...
- "${j.title || "Untitled"}" \u2014 Status: ${j.status || "unknown"}`;
      ...
      if (history?.length > 0) {
        ...
        history.reverse().forEach((h) => {
          ...
          let who = h.direction === "outbound" ? handler === "team_manual" ? "NUI Team (human)" : "Monty (you)" : "Client";
          conversationHistory += `
...
//# sourceMappingURL=sms-monty.js.map
