// netlify/functions/monty-followup.js
var OPENPHONE_API_KEY = process.env.OPENPHONE_API_KEY;
var FROM_NUMBER = process.env.OPENPHONE_PHONE_NUMBER;
var SUPABASE_URL = process.env.SUPABASE_URL;
var SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY;
var ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY;
var CALENDLY_URL = process.env.CALENDLY_URL || "https://newurbaninfluence.com/contact";
var sbH = () => ({ "apikey": SUPABASE_KEY, "Authorization": `Bearer ${SUPABASE_KEY}`, "Content-Type": "application/json" });
var FOLLOWUP_ANGLES = [
...
async function logSms(contactId, phone, message, eventType) {
  ...
  await Promise.all([
    ...
    contactId ? fetch(`${SUPABASE_URL}/rest/v1/activity_log`, { method: "POST", headers: sbH(), body: JSON.stringify({ contact_id: contactId, type: "sms", event_type: eventType, direction: "outbound", content: message, metadata: { ai_generated: true, model: "claude-sonnet-4-6" }, read: false, created_at: now }) }).catch(() => {
    }) : Promise.resolve()
  ]);
}
async function updateContact(contactId, updates) {
  await fetch(`${SUPABASE_URL}/rest/v1/crm_contacts?id=eq.${contactId}`, { method: "PATCH", headers: { ...sbH(), "Prefer": "return=minimal" }, body: JSON.stringify({ ...updates, last_activity_at: (/* @__PURE__ */ new Date()).toISOString() }) }).catch(() => {
  });
}
...
exports.handler = async function(event) {
  ...
  for (const t of thresholds) {
    const cutoff = new Date(now - t.hoursAgo * 36e5).toISOString();
  ...
  for (const w of dormantWindows) {
    const cutoffOld = new Date(now - w.daysAgo * 864e5).toISOString();
    const cutoffNew = new Date(now - w.maxDays * 864e5).toISOString();
...
//# sourceMappingURL=monty-followup.js.map
